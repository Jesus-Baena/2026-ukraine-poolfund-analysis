...
An explanatory companion article can be found [here](https://baena.ai/articles/uhf-twin-tiers).
:::
...
**Interactive Dashboard:** Explore this visualization interactively at [**shiny.baena.info/uhf-grant-size-2025**](https://shiny.baena.info/uhf-grant-size-2025/) where you can filter by organization type, explore individual organizations, and hover for details.
:::
...
## The Four Types, Operationalised
...
```{r typology-definition-table}
#| echo: false

typology_def <- tribble(
  ~Type, ~Rule, ~RiskProfile,
  "**D** — Hyperscalers",
  "UN Agency, or pre-war global revenue ≥ $50M",
  "UHF volume is absorbable; institutional scale ≫ grant scale",
  
  "**C** — Localisation cohort",
  "National NGO with first CBPF grant before 2022",
  "Pre-war Ukrainian track record; absorbing transformation",
  
  "**B** — Small-base pre-war",
...
knitr::kable(typology_def,
             col.names = c("Type", "Operational Rule", "Risk Signature"),
             escape = FALSE,
             caption = "Partner typology: rule-based classification covering all 94 Ukraine partners",
             align = c("l", "l", "l"))
```
...
```{r partner-typology-setup}
#| include: false

# ===================================================================
# PART 1: Get all Ukraine partners 2022-2025 with basic metadata
# ===================================================================

all_ukraine_partners <- project_summary %>%
  filter(PooledFundName == "Ukraine",
         AllocationYear %in% 2022:2025,
         !is.na(Budget), Budget > 0,
         !is.na(OrganizationName)) %>%
  ...
  summarise(
    ...
    n_grants_total = n(),
    total_usd = sum(Budget, na.rm = TRUE),
    .groups = "drop"
  )

# ===================================================================
# PART 2: Hardcoded Core-20 classification
# Using revenue data + known founding dates
# ===================================================================

# Organizations confirmed to be founded/established in 2022 or later
type_a_born_of_war <- c(
  "NEW WAY",
  "POSMISHKA UA",
  "Unity for the Future"
)

core20_classification <- tribble(
  ~OrganizationName,                                 ~Type, ~Founding_Rationale,
  "United Nations High Commissioner for Refugees",  "D", "UN Agency; $5.15B revenue",
  "Agency for Technical Cooperation and Development", "D", "Pre-war revenue €393.7M (>>$50M)",
  "Danish Refugee Council",                         "D", "Pre-war revenue 3.12B DKK (~$497M)",
  "Norwegian Refugee Council",                      "D", "Pre-war revenue 5.67B NOK (~$660M)",
  "International Rescue Committee, Inc.",           "D", "Pre-war revenue $1.16B",
  "Save the Children Fund",                         "D", "Pre-war revenue $2.2B",
  "People in Need",                                 "B", "Pre-2022 founding (Czech INGO); first UHF grant 2022+",
  "ICF Caritas Ukraine",                            "C", "National NGO; pre-2022 operational history",
  "Proliska",                                       "C", "National NGO; pre-2022 operational history",
  "Ukrainian Red Cross Society",                    "C", "National NGO; pre-2022 operational history",
  "Interagency Standing Committee Food Security",   "C", "National NGO; pre-2022 operational history",
  "NEW WAY",                                        "A", "Founded 2022; born of the war response",
  "POSMISHKA UA",                                   "A", "Founded 2022; born of the war response",
  "Unity for the Future",                           "A", "Founded 2022; born of the war response",
  "ACTED",                                          "D", "Pre-war revenue €393.7M parent",
  "DRC (Danish Refugee Council)",                   "D", "Pre-war revenue 3.12B DKK (~$497M)",
  "Stichting ZOA",                                  "B", "Pre-2022 founding (Dutch INGO); first UHF grant 2022+",
  "Dorcas Aid International Transcarpathia",        "B", "Pre-2022 founding; €24M parent",
  "Estonian Refugee Council",                       "B", "Pre-2022 founding; first UHF grant 2022+",
  "CORE Community Organized Relief Effort",         "D", "Pre-war revenue $122.1M"
)

# ===================================================================
# PART 3: Merge hardcoded classifications + apply rules for rest
# ===================================================================

partner_typology <- all_ukraine_partners %>%
  left_join(core20_classification, by = "OrganizationName") %>%
  mutate(
    Type = case_when(
      !is.na(Type) ~ Type,  # Use hardcoded Core-20 classification if available
      
      # Rule-based classification for non-Core-20 partners
      OrganizationType == "UN Agency" ~ "D",
      
      OrganizationType == "National NGO" & first_year < 2022 ~ "C",
      
      # Type A: confirmed born-of-war organizations (founded 2022+)
      OrganizationName %in% type_a_born_of_war ~ "A",
      
      # Type B: pre-2022 founding or pre-2022 INGO with first CBPF grant >= 2022
      OrganizationType == "International NGO" & first_year < 2022 ~ "B",
      
      # Orgs with first_year >= 2022 but not confirmed as born-of-war: classify as Type B
      # (these are pre-2022 operators getting first CBPF funding post-war)
      first_year >= 2022 ~ "B",
    ...
    Type_Label = case_when(
      Type == "D" ~ "Hyperscaler",
      Type == "C" ~ "Localisation cohort",
      Type == "B" ~ "Small-base pre-war",
      Type == "A" ~ "Born of the war",
      TRUE ~ Type
    )
  ) %>%
  select(OrganizationName, OrganizationType, Type, Type_Label, 
         first_year, n_grants_total, total_usd)

# ===================================================================
# PART 4: Summary statistics by type
# ===================================================================

typology_summary <- partner_typology %>%
  group_by(Type, Type_Label) %>%
  summarise(
    n_partners = n_distinct(OrganizationName),
    n_grants = sum(n_grants_total),
    total_usd = sum(total_usd),
    median_grant = median(total_usd / n_grants_total),
    mean_partner_usd = mean(total_usd),
    .groups = "drop"
  ) %>%
  mutate(
    pct_of_total = total_usd / sum(total_usd) * 100,
    mean_grant = total_usd / n_grants
  )
...
I specialize in task-based consulting services for data analytics. You can contact me at [baena.ai/contact](https://baena.ai/contact)
:::
