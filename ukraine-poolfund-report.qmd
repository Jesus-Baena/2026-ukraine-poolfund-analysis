...
Rather than focusing on the lineal logic of the project cycle (number of beneficiaries, project duration, funding executed, etc...) the report takes a step back to look at the overall funding flow and the characteristics of the grants, with a particular emphasis on the health sector.

The analysis is structured around three key questions:

What is the profile of the organization receiving UHF funding? How does it compare to the rest of the CBPF system? What are the implications for the health sector?
...
-   **Visible capacity-volume mismatches**: Where 2021 revenue is public, the Estonian Refugee Council has managed UHF volumes **4.3× its pre-war annual revenue** (\$9.4M against €1.9M). Seven of the eight core Ukrainian NNGOs have no publicly available 2021 revenue — an independent-assessment gap rather than a finding.
...
ggplot(conc_agg,
       aes(x = n_partners, y = HHI,
           size = envelope / 1e6, colour = Class, shape = is_ukraine)) +
  geom_point(alpha = 0.85, stroke = 1.2) +
  ggrepel::geom_text_repel(
    aes(label = PooledFundName,
        fontface = if_else(is_ukraine, "bold", "plain")),
    size = 3.6, box.padding = 0.45, seed = 7,
    show.legend = FALSE
  ) +
  scale_size_area(max_size = 18, name = "Envelope (US$M)",
                  breaks = c(150, 300, 500)) +
  scale_colour_manual(values = c(
    "Access-restricted (UN-dominant)" = "#e76f51",
    "Broad partner fund"              = "#0a9396"
  ), name = NULL) +
  scale_shape_manual(values = c(`TRUE` = 18, `FALSE` = 16),
                     guide = "none") +
  scale_x_continuous(breaks = c(4, 10, 25, 50, 75, 100, 125)) +
  labs(
    title    = "CBPF Concentration Map: Partners vs HHI (2022–2024 aggregate)",
    subtitle = "Bubble size = 3-year envelope. Ukraine is a broad fund with an outsized envelope — not an outlier in shape.",
    x = "Distinct implementing partners (3-year count)",
    y = "Herfindahl–Hirschman Index (partner-budget shares)",
    caption = "Source: CBPF ProjectSummary dataset, Feb 2026 extract"
  ) +
  theme_minimal(base_size = 13) +
  theme(
...
-   The **Full-coverage quadrant is mixed** — it is not exclusively INGO territory. NNGOs and UN agencies both appear in meaningful numbers, confirming that broad footprint is achievable across organisation types.
